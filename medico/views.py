...
def cadastro_medico(request):
    ...
    if request.method == "GET":
        ...
        ## pega o usuário logado e vê se existe na tabela o médico já registrado
        ## se já estiver registrado, não voltar para página de cadastro.
...
def abrir_horario(request):
    ...
    if request.method == "GET":
        ...
        datas_abertas = DatasAbertas.objects.all()
...
def finalizar_consulta(request, id_consulta):
    ...
    if request.user != consulta.data_aberta.user:
        messages.add_message(request, constants.ERROR, 'Esta consulta pertence a outro usuário.')
